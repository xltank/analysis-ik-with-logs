...
class LexemePath extends QuickSortSet implements Comparable<LexemePath> {
	...
	void printLexemePath(String prefix) {
		Cell c = this.getHead();
		String s = "";
		while (c != null) {
			Lexeme l = c.getLexeme();
			s += l.getBeginPosition() + "-" + l.getEndPosition() + ", ";
			c = c.getNext();
		}
		System.out.println(prefix + " lexemePath: " + s);
	}
	...
	boolean addCrossLexeme(Lexeme lexeme) {
		if (this.isEmpty()) {
			...
			// printLexemePath("addCrossLexeme(), first");
			return true;
			...
			// printLexemePath("addCrossLexeme(), cross");
			return true;

		} else {
			// printLexemePath("addCrossLexeme(), no cross");
			return false;
		}
	}
