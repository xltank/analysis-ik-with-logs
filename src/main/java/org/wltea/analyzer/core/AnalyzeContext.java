...
class AnalyzeContext {
	...
	int fillBuffer(Reader reader) throws IOException {
		...
		// 重置当前指针
		this.cursor = 0;
		System.out.println("fillBuffer: " + segmentBuff.toString());
		return readCount;
	}
	...
	void printOrgLexemes() {
		QuickSortSet.Cell c = this.orgLexemes.getHead();
		while (c != null) {
			int startPos = Math.min(c.getLexeme().getBeginPosition(), this.segmentBuff.length);
			int endPos = Math.min(c.getLexeme().getEndPosition(), this.segmentBuff.length);
			endPos = Math.max(startPos, endPos);
			System.out.println("orgLexemes: "+new String(segmentBuff).substring(startPos, endPos));
			c = c.getNext();
		}
	}
